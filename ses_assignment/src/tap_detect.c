...
static int gpio_init(void)
{
    ...
    int ret = gpio_pin_configure_dt(&int1_gpio, GPIO_INPUT); if (ret) { return ret; }
    ret = gpio_pin_interrupt_configure_dt(&int1_gpio, GPIO_INT_EDGE_RISING); if (ret) { return ret; }
...
int tap_detect_init(void)
{
    ...
    int ret = gpio_init(); if (ret) { return ret; }

    ret = lsm6dsox_init(); if (ret) { return ret; }
